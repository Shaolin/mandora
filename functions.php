...
function save_application_to_db($contact_form) {
    ...
    $extra = sanitize_textarea_field($data['your-message'] ?? '');
...
//     $cv_file = $files['your-cv'];

//     // If it's an array, get first file
//     if (is_array($cv_file)) {
//         $cv = $cv_file[0];
//     } else {
//         $cv = $cv_file;
...
if (!empty($files['your-cv'])) {
    ...
    if (file_exists($file)) {
        ...
        $filename = wp_unique_filename($upload_dir['path'], basename($file));

        $new_path = $upload_dir['path'] . '/' . $filename;

        // Move file safely
        if (@rename($file, $new_path)) {
            $cv = $upload_dir['url'] . '/' . $filename;
        }
    }
}
...
//         if ($cv) {
//             echo '<a href="' . esc_url($cv) . '" target="_blank">Download CV</a>';
//         } else {
//             echo 'No CV';
//         }
//     }
